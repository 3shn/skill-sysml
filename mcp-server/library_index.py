...
class LibraryIndex:
    def __init__(self, library_path: str):
        ...
        self._searchable_elements: list[tuple[str, str, Element]] = []
    ...
    def search(self, query: str, limit: int = 25) -> list[dict]:
        ...
        for name, qn, e in self._searchable_elements:
            if name == q:
                score = 100
    ...
    def get(self, qualified_name: str) -> dict | None:
        ...
        if e is None:
            ...
            if not hasattr(self, "_by_bare_name"):
                self._by_bare_name = {}
                for x in self.elements:
                    self._by_bare_name.setdefault(x.name.strip("'"), []).append(x)
            target_name = qualified_name.strip("'")
            cands = self._by_bare_name.get(target_name, [])
            if len(cands) == 1:
                e = cands[0]
